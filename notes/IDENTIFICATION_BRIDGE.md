...
In-paper anchor: `paper/BAUM_CONNES_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the topological-to-analytic assembly package through an admissible operator-theoretic closure architecture`.
...
- endpoint lock: `assembly_transfer`.
...
2. the endpoint-transfer lock `assembly_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BC` remains positive.
...
Payload: verify `assembly_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BC` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `assembly_transfer`
