...
In-paper anchor: `paper/BAUM_CONNES_CONJECTURE_PREPRINT.md` (`BC_G4`, `BC_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving persistence of the topological-to-analytic assembly package through an admissible operator-theoretic closure architecture`.
...
- endpoint-transfer lock: `assembly_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`BC_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad nonisomorphic Baum-Connes countermodels are excluded.
`BC_G5` closes when `assembly_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the analytic assembly endpoint class.
Together they feed the strict margin `M_BC`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `assembly_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `BC_G4` and `BC_G5` deliver the boundary object needed by the final margin.
...
- transfer gate: `BC_G5`
- transfer artifact key: `assembly_transfer`
