...
In-paper anchor: `paper/BAUM_CONNES_CONJECTURE_PREPRINT.md` (`BC_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the topological-to-analytic assembly package through an admissible operator-theoretic closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BC_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and equivariant windows do not collapse.
This is the first-failure capture contribution to `M_BC`.
...
If near-failure extraction and no-Zeno spacing hold, then `BC_G3` prevents hidden drift before rigidity is applied.
