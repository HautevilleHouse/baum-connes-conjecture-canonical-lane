...
In-paper anchor: `paper/BAUM_CONNES_CONJECTURE_PREPRINT.md` (`BC_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of the topological-to-analytic assembly package through an admissible operator-theoretic closure architecture`.
...
- projected core: the response sector controlled by `kappa_analytic`.
...
`BC_G1` closes when `kappa_analytic` satisfies the response-floor requirement: projected analytic assembly response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_BC`.
...
Payload: verify that all quantities used by `kappa_analytic` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_analytic` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_analytic`, then `BC_G1` supplies the projected/protected-core input to `M_BC`.

## Current Instantiation

- gate: `BC_G1`
- artifact key: `kappa_analytic`
